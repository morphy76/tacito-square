...
| Affects       | internal/keeper/adapters/outbound/nats/community_broadcaster.go, internal/agent/adapters/inbound/nats/echo_subscriber.go, internal/keeper/adapters/inbound/http/echo_handlers.go |
...
1. When the Keeper's NATS broadcaster sends an echo request, it MUST initialize
   `nats.Msg.Header`, inject the W3C `traceparent` (and `tracestate`) using the global OTel
   propagator, and wrap the `nc.RequestMsgWithContext` call in a `SpanKindClient` child span
   named `"nats.request"` with attributes `messaging.system = "nats"`,
   `messaging.destination = <subject>`.
2. The broadcaster MUST record the NATS round-trip duration in
   `observability.OutboundDependencyDuration` with label `dependency = "nats"`.
3. When the Agent's echo subscriber receives a NATS message, it MUST extract the OTel context
   from `msg.Header` using the global propagator, start a `SpanKindConsumer` child span named
   `"nats.echo_handler"`, and use that context for all logging and downstream calls.
...
1. Sending a `POST /api/v1/communities/:community_id/echo` request and inspecting Zipkin shows
   a complete trace tree: `keeper.echo_community` (server) → `nats.request` (client) →
   `nats.echo_handler` (consumer on the agent).
2. Agent pod logs for an echo request contain `"trace_id"` and `"span_id"` fields matching the
   originating HTTP trace.
3. The `outbound_dependency_duration_seconds` Prometheus histogram includes samples with
   `dependency="nats"`.
4. The keeper HTTP span name in Zipkin is `"keeper.echo_community"`.
5. All existing unit tests for `community_broadcaster.go` and `echo_subscriber.go` remain GREEN.
