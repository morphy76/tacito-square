...
* Redirect URIs MUST be handled by the BFF at `/api/bff/v1/auth/callback`.
...
* **UserInfo Endpoint**: The BFF MUST expose `GET /api/bff/v1/auth/me`. This endpoint returns the user profile attributes, tenant IDs, and RBAC roles retrieved from Keycloak's UserInfo endpoint. The frontend UIs use this payload to drive role-based UI rendering.
...
* The BFF MUST expose `POST /api/bff/v1/auth/backchannel-logout`.
...
2. **Role Verification**: The UIs drive visual authorization checks based entirely on the JSON payload returned from `GET /api/bff/v1/auth/me`.
...
* **User Profile Retrieval**: Test that `/api/bff/v1/auth/me` returns the cached UserInfo correctly.
